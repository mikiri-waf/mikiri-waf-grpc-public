...
#include "mikiri_waf_grpc_module.h"
...
static ngx_int_t ngx_http_waf_grpc_body_output_filter(void *data, ngx_chain_t *in);
...
static ngx_int_t ngx_http_waf_grpc_filter(void *data, ssize_t bytes);
...
static char *ngx_http_waf_grpc_pass(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf);
ngx_int_t ngx_http_grpc_waf_init(ngx_conf_t *cf);
...
ngx_module_t  mikiri_waf_grpc_module;
extern ngx_module_t  ngx_http_grpc_module;

static ngx_command_t  ngx_http_waf_grpc_commands[] = {

    { ngx_string("waf_grpc_pass"),
      NGX_HTTP_LOC_CONF|NGX_HTTP_LIF_CONF|NGX_CONF_TAKE1,
      ngx_http_waf_grpc_pass,
...
static ngx_http_module_t  ngx_http_waf_grpc_module_ctx = {
    NULL,                                  /* preconfiguration */
    ngx_http_grpc_waf_init,                /* postconfiguration */
...
ngx_module_t  mikiri_waf_grpc_module = {
    NGX_MODULE_V1,
    &ngx_http_waf_grpc_module_ctx,             /* module context */
    ngx_http_waf_grpc_commands,                /* module directives */
...
ngx_int_t ngx_http_grpc_waf_init(ngx_conf_t *cf) {
  ...
  wmc = ngx_http_conf_get_module_main_conf(cf, mikiri_waf_http_module);
  wmc->grpc_module = 1;
...
static ngx_int_t
ngx_http_waf_grpc_handler(ngx_http_request_t *r)
{
    ngx_int_t                  rc;
    ngx_http_upstream_t       *u;
    ngx_http_grpc_ctx_t       *ctx;
    ...
    ngx_http_waf_grpc_ctx_t  *waf_grpc_ctx;

    rctx = ngx_http_get_module_ctx(r, mikiri_waf_http_module);
    if (rctx != NULL) {
      if (rctx->delayed || rctx->icap_delayed) {
        /*
         * the external handlers have not finished yet; keep the request
         * alive, the content phase is re-entered once they are done
         */
    ...
    waf_grpc_ctx = ngx_http_get_module_ctx(r, mikiri_waf_grpc_module);
    if (waf_grpc_ctx == NULL) {
      waf_grpc_ctx = ngx_pcalloc(r->pool, sizeof(ngx_http_waf_grpc_ctx_t));
      if (waf_grpc_ctx == NULL) {
        return NGX_HTTP_INTERNAL_SERVER_ERROR;
      }

      waf_grpc_ctx->header_sent = 0;
      waf_grpc_ctx->in = NULL;
      waf_grpc_ctx->request = r;
      ngx_http_set_ctx(r, waf_grpc_ctx, mikiri_waf_grpc_module);
    }
    ...
    u->input_filter = ngx_http_waf_grpc_filter;
...
ngx_http_grpc_create_request(ngx_http_request_t *r)
{
    ...
    u->output.output_filter = ngx_http_waf_grpc_body_output_filter;
...
static ngx_int_t
ngx_http_waf_grpc_body_output_filter(void *data, ngx_chain_t *in)
{
    ngx_http_request_t  *r = data;
    ngx_http_grpc_ctx_t    *ctx;
    ngx_int_t              waf_result;
    ngx_http_request_ctx_t    *rctx;
    ngx_http_waf_grpc_ctx_t       *waf_grpc_ctx;
    ngx_chain_t *n_in;
    ...
    if ((waf_grpc_check_flags(r) == NGX_DECLINED) || (waf_grpc_ctx->data_parsed == 1)) {
    ...
    if (in) {
      if ((ngx_buf_size(in->buf) != 0) || ngx_buf_special(in->buf)){
        if (ngx_chain_add_copy(r->pool, &waf_grpc_ctx->in, in) != NGX_OK) {
          return NGX_ERROR;
        }
      }
    }

    if (n_in && !waf_grpc_ctx->header_sent) {
      /* the first buffer holds the HTTP/2 headers frame, skip it */
      waf_grpc_ctx->header_sent = 1;
      n_in = n_in->next;
    }
    ...
    if (rctx->delayed || rctx->icap_delayed) {
      ...
      if ((rctx->mla_send_error == 1) || (rctx->icap_send_error == 1)) {
        if (rctx->block == WAF_BLOCK_BY_INT_ERR) {
          ctx->output_blocked = 1;
          ctx->done = 1;
        ...
        if ((rctx->block == WAF_BLOCK_BY_MLA) || (rctx->block == WAF_BLOCK_BY_AV)) {
          ctx->output_blocked = 1;
          ctx->done = 1;
        ...
        if (waf_grpc_ctx->external_handlers == 1) {
          waf_grpc_ctx->external_handlers = 0;
          waf_grpc_ctx->data_parsed = 1;
          ctx->output_blocked = 0;
        }
      }
    }

    if (waf_grpc_ctx->data_parsed == 0){
      if (n_in != NULL){
        if ((waf_grpc_ctx->header_sent == 1) && (!ngx_buf_special(n_in->buf))) {

          waf_result = waf_grpc_process_request(n_in->buf, r);
          if ((waf_result != NGX_OK) && (waf_result != NGX_DECLINED)) {
            if (waf_result == NGX_AGAIN) {
              /* the body has been passed to the external handlers,
               * wait for their verdict */
              waf_grpc_ctx->external_handlers = 1;
              ctx->output_blocked = 1;
    ...
    if (waf_grpc_ctx->data_parsed == 1) {
      waf_chain_remove_empty_buf(&(waf_grpc_ctx->in));
      return ngx_http_grpc_body_output_filter(data, waf_grpc_ctx->in);
    }
...
ngx_http_grpc_body_output_filter(void *data, ngx_chain_t *in)
{
    ...
    while (in && limit > 0) {
        ...
        do {
            ...
            *ll = cl;
            ...
            b->tag = (ngx_buf_tag_t) &ngx_http_waf_grpc_body_output_filter;
            b->shadow = in->buf;
            b->last_shadow = next;
    ...
    *ll = NULL;
    ...
    ngx_chain_update_chains(r->pool, &ctx->free, &ctx->busy, &out,
                           (ngx_buf_tag_t) &ngx_http_waf_grpc_body_output_filter);
...
static ngx_int_t
ngx_http_waf_grpc_filter(void *data, ssize_t bytes)
{
    ngx_http_grpc_ctx_t  *ctx = data;
    ...
    if (rctx != NULL && waf_grpc_ctx != NULL
        && (rctx->delayed || rctx->icap_delayed))
    {
        /*
         * the external handlers are still running; keep the received data
         * in the buffer and parse it once they are done
         */

        ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                       "grpc filter delayed bytes:%z", bytes);

        if (!waf_grpc_ctx->filter_delayed) {
            waf_grpc_ctx->filter_delayed = 1;
            b->pos = b->last;
        }
    ...
    if (waf_grpc_ctx != NULL && waf_grpc_ctx->filter_delayed) {
        /* b->pos already points to the data postponed while delayed */
        waf_grpc_ctx->filter_delayed = 0;
...
ngx_http_grpc_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    {
        clcf->handler = ngx_http_waf_grpc_handler;
    }
...
static char *
ngx_http_waf_grpc_pass(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_http_grpc_loc_conf_t   *glcf;
    ...
    glcf = ngx_http_conf_get_module_loc_conf(cf, ngx_http_grpc_module);
